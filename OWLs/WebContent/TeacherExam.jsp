...
onclick="
var link = `${document.location.origin}/OWLs/TeacherSubjectDisplay.jsp?`;
...
function load(){
	...
	addButton.addEventListener("click", function(e){
		var link = `${document.location.origin}/OWLs/TeacherExamDetail.jsp?subjectCode=` +  e.target.getAttribute("data-value")
		window.location = link
	})
	...
	for (var i=0; i<deleteButtons.length; i++){
		deleteButtons[i].addEventListener("click", function(e){
			var link = `${document.location.origin}/OWLs/Delete.jsp?`
	...
	for (var i=0; i<editButtons.length; i++){
		editButtons[i].addEventListener("click", function(e){
			var link = `${document.location.origin}/OWLs/TeacherExamDetailEdit.jsp?`
			var params = "subjectCode=" + e.target.getAttribute("data-subject-id") + 
			"&year=" + e.target.getAttribute("data-year") + 
			"&semester=" + e.target.getAttribute("data-semester") + 
			"&examType=" + e.target.getAttribute("data-exam-type")
	...
	for (var i=0; i<editQuestions.length; i++){
		editQuestions[i].addEventListener("click", function(e){
			var link = `${document.location.origin}/OWLs/TeacherExamQuestions.jsp?`
			var params = "subjectCode=" + e.target.getAttribute("data-subject-id") + 
			"&year=" + e.target.getAttribute("data-year") + 
			"&semester=" + e.target.getAttribute("data-semester") + 
			"&examType=" + e.target.getAttribute("data-exam-type")
